...
instruction = (
        ...
        "Если задан определённый стиль (например, «в стиле Властелина колец», «как у Губки Боба» и т.д.),\n"
        "обязательно используй характерные фразы, пасхалки, атмосферу или упоминания,\n"
        "чтобы стиль был узнаваемым для читателя. Это могут быть:\n"
        "- имена персонажей,\n"
        "- цитаты или адаптированные обороты речи,\n"
        "- особенности языка и поведения героев,\n"
        "- узнаваемые ситуации или локации из мира, с которым связан стиль.\n"
        "Перед выдачей ответа обязательно проверяй логичность, грамматическую и синтаксическую правильность текста."
)


def generate_congrat_gpt_3_5(data: dict | None = None, prompt: str | None = None, regenerate: bool = False) -> dict:
...
def generate_congrat_gpt_4_1(data: dict | None = None, prompt: str | None = None, regenerate: bool = False) -> dict:
    # Вывод: {'congrat_type': 'achievement', 'achieve': 'победа на
    # соревнованиях по плаванию', 'congrat_recipient_role': 'friend_boy',
    # 'congrat_style': 'spiritual', 'reciever_name': 'Артём'}
    # Загружаем локальные переменные
